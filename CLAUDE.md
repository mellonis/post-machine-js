...
6. **Per-State debug lockdown (v6.3.0+)**: at the end of construction, `PostMachine` iterates `#stateToCandidatePaths.keys()` and calls `installStateLockdown(state, onUserWrite)` on every non-halt State. The installer replaces the engine's prototype `debug` accessor with an instance-level `Object.defineProperty`. Internal writes (from `#refreshStateDebug` / `#refreshHaltDebug`) run inside `withLockdownEscape` and delegate to the engine's prototype setter (which preserves the engine's `DebugConfig` wrapping + validation + shared-debugRef propagation across `withOverrodeHaltState` wrappers). User writes outside the escape go through the redirect handler: un-shared State → `setBreakpoint`/`clearBreakpoint`; shared State → throw with candidate-path list. `haltState` is locked module-globally in `src/index.ts` at module load — direct `haltState.debug = X` throws, no PostMachine context for a redirect. `state.isHalt` checks at the install site skip the engine's halt singleton (it has its own module-global lockdown). The lockdown does **not** use `Proxy` — that was tried in the v6.3.0 design phase and abandoned because engine utilities like `State.toGraph(arg, …)` read TS-downleveled private fields directly off the argument via `__classPrivateFieldGet`, which fails on a Proxy.
...
- **`onPause` on `pm.run()`** (stable as of v6.2.0; was experimental `__onPause` in earlier v6 drafts, renamed when the debugger surface settled). Accepts `onPause?: (s: MachineState) => void | Promise<void>` and forwards as the upstream `onPause` hook. In v6.3.0+ the wrapper applies arrival-aware registry filtering: pauses fire only when `m.arrivalPath` (or a halt-arrival) matches a registered breakpoint.
- **Debugger primitives ARE wrapped (v6.3.0+).** `state.debug` and `haltState.debug` go through the per-State / module-global lockdown (see Subtlety 6 above). Construction-time writes funnel through `pm.setBreakpoint(target, filter)` / `pm.clearBreakpoint(target)` / `pm.clearBreakpoints()`; direct `state.debug = X` on an un-shared State auto-redirects to `setBreakpoint`. The engine-level concepts (filter shapes, `before → step → after` lifecycle, `haltState.debug.after` rejection) still apply — the lockdown is a thin layer in front, not a reimplementation.
